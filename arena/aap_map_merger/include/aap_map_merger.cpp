...
void aap_map_merger::inicializer(void)
{
	...
	for (int i = 0; i < number_of_robots; i++){
		...
		robots_map_topics[i] = "/" + name_string +  "/map";
	}
	...
	global_map_pub = n.advertise<nav_msgs::OccupancyGrid>(global_map_topic,map_buffer,true);
...
void aap_map_merger::startSubscribing(void)
{
	...
	for(int i = 0; i<number_of_robots;i++)
		map_sub[i] = n.subscribe<nav_msgs::OccupancyGrid>(robots_map_topics[i] , map_buffer, &aap_map_merger::mapReceived, this);
